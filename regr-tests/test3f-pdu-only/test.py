...
class TestMuxs(tu.Muxs):
    ...
    async def handle_updf(self, ws, msg):
        ...
        if 'pdu' in msg and 'MHdr' not in msg and 'DevAddr' not in msg:
            logger.info('PDU-only mode verified successfully')
    ...
    def get_router_config(self):
        ...
        with open('debug.log', 'a') as f:
            f.write('TEST: Sending router config with PDU-only enabled\n')
        return config
