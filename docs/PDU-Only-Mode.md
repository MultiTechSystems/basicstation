...
When both PDU-only mode and protobuf encoding are enabled, message sizes are significantly reduced.

### Configuration

Enable both features in `router_config`:
...
For a typical 24-byte LoRaWAN uplink frame:

| Format | Size | Reduction vs JSON Parsed |
|--------|------|--------------------------|
| JSON (parsed fields) | ~325 bytes | baseline |
| JSON (pdu-only hex) | ~254 bytes | 22% smaller |
| JSON (pdu-only base64) | ~238 bytes | 27% smaller |
| Protobuf (parsed fields) | ~90 bytes | 72% smaller |
| **Protobuf (pdu-only)** | **~80 bytes** | **75% smaller** |

### Protobuf PDU-Only Format

In protobuf mode, PDU-only uses the `pdu` field in the `UplinkDataFrame` message:

```protobuf
message UplinkDataFrame {
  // Parsed fields (mhdr, dev_addr, etc.) - omitted in pdu-only mode
  ...
  
  // Raw PDU (PHYPayload) - used in pdu-only mode
  bytes pdu = 11;
  
  // Radio metadata - always included
...
When `pdu_only: true`:
- The `pdu` field contains the raw PHYPayload as binary bytes
- Parsed fields (mhdr, dev_addr, fctrl, fcnt, fopts, fport, frm_payload, mic) are omitted
- Radio metadata (upinfo) and ref_time are always included

### Benefits

1. **Maximum bandwidth savings**: 75% reduction vs JSON parsed mode
2. **No encoding overhead**: PDU stored as raw bytes (not hex/base64)
3. **Lower CPU usage**: No frame parsing on gateway, no string encoding
4. **Ideal for constrained backhaul**: Cellular, satellite, or metered connections

### Implementation

The station uses `tcpb_encUpdfPduOnly()` when both protobuf and pdu-only are enabled:

```c
// In s2e.c uplink path
if (tcpb_enabled() && s2e_pduOnly) {
    pblen = tcpb_encUpdfPduOnly(
        buf, bufsize,
        pdu, pdulen,          // Raw PHYPayload
        dr, freq,
        rctx, xtime, gpstime,
        rssi, snr, fts,
        rxtime, reftime);
...
### Test Results (Actual Measured Sizes)

From selftest_tcpb.c:

| Message Type | Protobuf Size |
|--------------|---------------|
| updf (minimal) | 90 bytes |
| updf (full) | 108 bytes |
| updf (max payload) | 327 bytes |
| jreq | 100 bytes |
| propdf | 73 bytes |
| dntxed | 42 bytes |
| timesync | 13 bytes |
...
For a gateway with 10,000 uplinks/day:

| Format | Daily Bandwidth |
|--------|-----------------|
| JSON parsed | ~3.25 MB |
| JSON pdu-only (hex) | ~2.54 MB |
| JSON pdu-only (base64) | ~2.38 MB |
| Protobuf parsed | ~0.90 MB |
| Protobuf pdu-only | ~0.80 MB |

**Monthly savings**: ~73 MB vs JSON parsed (97% reduction)
